                      ...
                      const nr_rlc_configuration_t *default_rlc_config)
{
  ...
  if (RC.nb_nr_macrlc_inst > 0) {
    ...
    for (module_id_t i = 0; i < RC.nb_nr_macrlc_inst; i++) {
      ...
      init_blocked_prb_mask();
      RC.nrmac[i]->ul_beam_alloc = nr_ul_beam_alloc_default;
      RC.nrmac[i]->ul_sched_policy = nr_ul_proportional_fair;
      const char *lua_ul_path = getenv("LUA_SCHED_UL");
      AssertFatal(lua_ul_path && lua_ul_path[0] != '\0',
                  "LUA_SCHED_UL environment variable must be set to the path of the Lua UL scheduling script\n");
