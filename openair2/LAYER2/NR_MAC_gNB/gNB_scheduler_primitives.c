...
#define BLER_UPDATE_FRAME 100
#define BLER_FILTER 0.7f
                      ...
                      frame_t frame)
{
  ...
  const float bler_window = num_dl_sched > 0 ? (float) num_dl_retx / num_dl_sched : -1.0f;
  if (bler_window >= 0.0f) // only update BLER average when we have actual data
  ...
  if (num_dl_sched > 0) {
    if (bler_stats->bler < bler_options->lower && old_mcs < max_mcs)
      new_mcs += 1;
    else if (bler_stats->bler > bler_options->upper)
      new_mcs -= 1;
    // else we are within threshold boundaries
  }
  // no MCS change if insufficient data to make a decision
...
NR_UE_info_t *find_nr_UE_by_uid(NR_UEs_t *UEs, unsigned int uid)
{
  UE_iterator(UEs->connected_ue_list, UE) {
    if (UE->uid == uid)
      return UE;
...
/* ============ PRB Blocking Infrastructure ============ */

char g_blocked_prb_mask[MAX_BWP_SIZE + 1];

void init_blocked_prb_mask(void)
{
  memset(g_blocked_prb_mask, '.', MAX_BWP_SIZE);
  g_blocked_prb_mask[MAX_BWP_SIZE] = '\0';
}

void set_blocked_prb_mask(const char *mask)
{
  int len = strlen(mask);
  if (len > MAX_BWP_SIZE)
    len = MAX_BWP_SIZE;
  memcpy(g_blocked_prb_mask, mask, len);
  g_blocked_prb_mask[len] = '\0';
}

void block_prb_range(int start_prb, int num_prbs)
{
  for (int i = start_prb; i < start_prb + num_prbs && i < MAX_BWP_SIZE; i++)
    g_blocked_prb_mask[i] = 'X';
}

void unblock_prb_range(int start_prb, int num_prbs)
{
  for (int i = start_prb; i < start_prb + num_prbs && i < MAX_BWP_SIZE; i++)
    g_blocked_prb_mask[i] = '.';
}

void clear_blocked_prb_mask(void)
{
  memset(g_blocked_prb_mask, '.', MAX_BWP_SIZE);
  g_blocked_prb_mask[MAX_BWP_SIZE] = '\0';
}
...
void set_ue_blocked_prb_mask(NR_UE_info_t *UE, const char *mask)
{
  int len = strlen(mask);
  if (len > MAX_BWP_SIZE)
    len = MAX_BWP_SIZE;
  memcpy(UE->ul_blocked_prb_mask, mask, len);
  UE->ul_blocked_prb_mask[len] = '\0';
}

static void ensure_ue_mask_initialized(NR_UE_info_t *UE)
{
  if (UE->ul_blocked_prb_mask[0] == '\0') {
    memset(UE->ul_blocked_prb_mask, '.', MAX_BWP_SIZE);
    UE->ul_blocked_prb_mask[MAX_BWP_SIZE] = '\0';
  }
}

void block_ue_prb_range(NR_UE_info_t *UE, int start_prb, int num_prbs)
{
  ensure_ue_mask_initialized(UE);
  for (int i = start_prb; i < start_prb + num_prbs && i < MAX_BWP_SIZE; i++)
    UE->ul_blocked_prb_mask[i] = 'X';
}

void unblock_ue_prb_range(NR_UE_info_t *UE, int start_prb, int num_prbs)
{
  ensure_ue_mask_initialized(UE);
  for (int i = start_prb; i < start_prb + num_prbs && i < MAX_BWP_SIZE; i++)
    UE->ul_blocked_prb_mask[i] = '.';
}

void clear_ue_blocked_prb_mask(NR_UE_info_t *UE)
{
  memset(UE->ul_blocked_prb_mask, '.', MAX_BWP_SIZE);
  UE->ul_blocked_prb_mask[MAX_BWP_SIZE] = '\0';
}
