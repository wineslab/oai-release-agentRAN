-- UL Proportional Fair Scheduler with Pipe-based Metrics Logging
-- Optimized for real-time: single batched write per slot
--
...
local LOG_EVERY_N_SLOTS = 200           -- Log scheduler metrics every Nth slot
...
local function pipe_write_nonblock(data)
    ...
    if ret < 0 then
        ...
        if err == EAGAIN then
            ...
            print("[UL Scheduler] Pipe write error: " .. ffi.string(ffi.C.strerror(err)))
            ffi.C.close(pipe_fd)
            pipe_fd = -1
...
local function pipe_write_buffered(data)
    ...
    while #overflow_buffer > MAX_OVERFLOW_SIZE do
        ...
        if overflow_drops % 1000 == 0 then
            print("[UL Scheduler] Warning: dropped " .. overflow_drops .. " messages (pipe reader too slow)")
        end
    end
end
...
local function ensure_pipe_open()
    ...
    if fd >= 0 then
        ...
        overflow_buffer = {}
        overflow_drops = 0
        print("[UL Scheduler] Connected to metrics pipe (non-blocking): " .. PIPE_PATH)
        return true
    else
        if not pipe_open_attempted then
            print("[UL Scheduler] Pipe not available: " .. PIPE_PATH .. " (create with: mkfifo " .. PIPE_PATH .. ")")
...
local function log_metrics_batched(metrics, n_ues, frame, slot, ts, include_channel_mag, rb_mask)
    ...
    -- Header: timestamp, frame, slot, PRB mask
    if rb_mask then
    ...
    for i = 0, n_ues - 1 do
        ...
        -- Only include every LOG_CHANNEL_MAG_EVERY_N slots to reduce CPU overhead
...
-- Budget accumulates at target rate, each allocation deducts estimated bytes.
-- The scale factor self-calibrates by comparing estimated vs actual throughput
-- (m.throughput is in bps, from C-side EWMA updated once per 10ms frame).
...
-- Local PF throughput EWMA per UE (decays for unscheduled UEs)
local ue_pf_tp = {}        -- rnti -> {tp, last_time}
local PF_EWMA_ALPHA = 0.001 -- smoothing per slot (~500-slot window)

local function allocate_new_data_ues(metrics, n_ues, rb_mask_string, min_rbs)
    local current_mask = rb_mask_string
    local now = socket.gettime()

    -- Collect new data UEs and compute PF metrics using local EWMA
    local new_data_ues = {}
    for i = 0, n_ues - 1 do
        local m = metrics[i]
        if m.ue_type == UE_TYPE_NEW_DATA then
            ...
            if not state then
                state = {tp = 1.0, last_time = now}
                ue_pf_tp[rnti] = state
            end
            -- Decay toward 0: unscheduled UEs see tp shrink every slot
            state.tp = state.tp * (1 - PF_EWMA_ALPHA)
            if state.tp < 1.0 then state.tp = 1.0 end
            local pf_metric = (m.previous_mcs + 1) / state.tp
...
function compute_allocations(metrics_ptr, n_ues, total_rbs, min_rbs, rb_mask_string)
    ...
    if slot_counter >= LOG_EVERY_N_SLOTS then
        ...
        log_metrics_batched(metrics, n_ues, frame, slot, ts, false, rb_mask_string)
    end
