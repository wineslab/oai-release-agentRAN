...
int nr_find_delay_candidates(int ofdm_symbol_size,
                             const c16_t *ch_estimates_time,
                             int max_val,
                             delay_candidate_t *candidates,
                             int max_candidates)
{
  if (max_val == 0 || max_candidates <= 0)
    return 0;

  const int threshold = max_val >> 2; // -6 dB relative to peak
  const int half = ofdm_symbol_size / 2;
  int num_found = 0;

  for (int i = 0; i < ofdm_symbol_size && num_found < max_candidates; i++) {
    int power = c16amp2(ch_estimates_time[i]) >> 1;
    if (power <= threshold)
      continue;

    // Check if local maximum (stronger than both neighbors)
    int prev_idx = (i == 0) ? ofdm_symbol_size - 1 : i - 1;
    int next_idx = (i == ofdm_symbol_size - 1) ? 0 : i + 1;
    int prev_power = c16amp2(ch_estimates_time[prev_idx]) >> 1;
    int next_power = c16amp2(ch_estimates_time[next_idx]) >> 1;
    if (power < prev_power || power < next_power)
      continue;

    // Wrap position to [-N/2, N/2)
    int pos = (i > half) ? i - ofdm_symbol_size : i;

    // Check minimum separation from all existing candidates
    int too_close = 0;
    for (int j = 0; j < num_found; j++) {
      int existing_raw = candidates[j].position < 0 ? candidates[j].position + ofdm_symbol_size : candidates[j].position;
      int dist = abs(i - existing_raw);
      if (dist > half)
        dist = ofdm_symbol_size - dist;
      if (dist < MIN_CANDIDATE_SEPARATION) {
        // Keep the stronger one
        if (power > candidates[j].power) {
          candidates[j].position = pos;
          candidates[j].power = power;
        }
        too_close = 1;
        break;
      }
    ...
    // Insert sorted by power (descending) using insertion sort
    int insert_pos = num_found;
    for (int j = 0; j < num_found; j++) {
      if (power > candidates[j].power) {
        insert_pos = j;
        break;
    ...
    // Shift elements down
    for (int j = num_found; j > insert_pos; j--)
      candidates[j] = candidates[j - 1];
    candidates[insert_pos].position = pos;
    candidates[insert_pos].power = power;
    num_found++;
  }
