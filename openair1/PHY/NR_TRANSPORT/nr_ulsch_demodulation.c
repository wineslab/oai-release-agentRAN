...
static uint8_t nr_ulsch_mmse_2layers(int **rxdataF_comp,
                                     ...
                                     unsigned char symbol,
                                     int length,
                                     const uint32_t *noise_var_per_rb)
{
  ...
  // Add noise_var such that: H^h * H + noise_var * I (per-RB regularization)
  if (noise_var_per_rb != NULL) {
    ...
    for (int rb = 0; rb < nb_rb_0; rb++) {
      simde__m128i nvar_128i = simde_mm_set1_epi32(noise_var_per_rb[rb]);
      for (int j = 0; j < 3; j++) {  // 12 subcarriers / 4 per SIMD = 3
                     ...
                     c16_t *chFext_slot)
{
  ...
  int dmrs_symbol;
  if (gNB->chest_time == 0) {
    // Count DMRS symbols to check if time-domain interpolation was applied
    int end_sym = rel15_ul->start_symbol_index + rel15_ul->nr_of_symbols;
    int n_dmrs = 0;
    for (int l = rel15_ul->start_symbol_index; l < end_sym; l++)
      n_dmrs += (rel15_ul->ul_dmrs_symb_pos >> l) & 1;
    if (n_dmrs > 1)
      // Interpolated estimates are at each symbol's position
      dmrs_symbol = symbol;
    else
    ...
    int end_sym = rel15_ul->start_symbol_index + rel15_ul->nr_of_symbols;
    dmrs_symbol = get_next_dmrs_symbol_in_slot(rel15_ul->ul_dmrs_symb_pos, rel15_ul->start_symbol_index, end_sym);
  }
  ...
  // Per-RB LLR noise scaling for single-layer: normalize LLR confidence by per-RB noise
  if (nb_layer == 1 && nvar_per_rb != NULL) {
    uint64_t nvar_sum = 0;
    for (int rb = 0; rb < rel15_ul->rb_size; rb++)
      nvar_sum += nvar_per_rb[rb];
    uint32_t nvar_avg = (uint32_t)(nvar_sum / rel15_ul->rb_size);

    if (nvar_avg > 0) {
      int llrs_per_re = rel15_ul->qam_mod_order;
      int nb_re_per_rb = length < 12 * (int)rel15_ul->rb_size ? length / (int)rel15_ul->rb_size : 12;
      for (int aatx = 0; aatx < nb_layer; aatx++) {
        int16_t *llr_ptr = llr[aatx];
        for (int rb = 0; rb < (int)rel15_ul->rb_size; rb++) {
          int scale_q8;
          if (nvar_per_rb[rb] > 0)
            scale_q8 = (int)(((uint64_t)nvar_avg << 8) / nvar_per_rb[rb]);
          else
            scale_q8 = 256;
          if (scale_q8 > 1024)
            scale_q8 = 1024; // clamp to 4.0
          if (scale_q8 != 256) {
            int llr_count = nb_re_per_rb * llrs_per_re;
            for (int i = 0; i < llr_count; i++) {
              int idx = rb * llr_count + i;
              llr_ptr[idx] = (int16_t)((llr_ptr[idx] * scale_q8) >> 8);
            }
          }
        }
      }
...
typedef struct puschSymbolProc_s {
  ...
  const uint32_t *nvar_per_rb;  // per-RB noise variance [rb_size], NULL for single-layer
  int beam_nb;
  task_ans_t *ans;
...
// Linear interpolation of channel estimates between DMRS symbols in time.
// For data symbols bracketed by two DMRS symbols, linearly interpolates.
// For data symbols outside the DMRS span, copies the nearest DMRS estimate.
static void nr_chest_time_domain_interp(int32_t **ch_estimates,
                                        ...
                                        int nb_rb_pusch)
{
  int end_symbol = start_symbol + num_symbols;
  int num_re = nb_rb_pusch * NR_NB_SC_PER_RB;

  // Collect DMRS symbol positions
  int dmrs_syms[14];
  int num_dmrs = 0;
  for (int s = start_symbol; s < end_symbol; s++)
    if ((dmrs_bitmap >> s) & 1)
      dmrs_syms[num_dmrs++] = s;

  if (num_dmrs < 1)
    return;

  int num_ch = nrOfLayers * nb_antennas_rx;
  for (int s = start_symbol; s < end_symbol; s++) {
    if ((dmrs_bitmap >> s) & 1)
      continue; // DMRS symbol: estimates already computed

    // Find bracketing DMRS symbols
    int prev_dmrs = -1, next_dmrs = -1;
    for (int d = 0; d < num_dmrs; d++) {
      if (dmrs_syms[d] < s)
        prev_dmrs = dmrs_syms[d];
      if (dmrs_syms[d] > s && next_dmrs == -1)
        next_dmrs = dmrs_syms[d];
    }

    for (int ch = 0; ch < num_ch; ch++) {
      c16_t *dst = (c16_t *)&ch_estimates[ch][s * ofdm_symbol_size];

      if (prev_dmrs >= 0 && next_dmrs >= 0) {
        // Interpolate between two DMRS symbols
        c16_t *h0 = (c16_t *)&ch_estimates[ch][prev_dmrs * ofdm_symbol_size];
        c16_t *h1 = (c16_t *)&ch_estimates[ch][next_dmrs * ofdm_symbol_size];
        int span = next_dmrs - prev_dmrs;
        int w1 = s - prev_dmrs;
        int w0 = span - w1;
        for (int k = 0; k < num_re; k++) {
          dst[k].r = (int16_t)((h0[k].r * w0 + h1[k].r * w1) / span);
          dst[k].i = (int16_t)((h0[k].i * w0 + h1[k].i * w1) / span);
        }
      } else if (prev_dmrs >= 0) {
        // After last DMRS: use nearest
        memcpy(dst, &ch_estimates[ch][prev_dmrs * ofdm_symbol_size], num_re * sizeof(c16_t));
      } else {
        // Before first DMRS: use nearest
        memcpy(dst, &ch_estimates[ch][next_dmrs * ofdm_symbol_size], num_re * sizeof(c16_t));
      }
    }
  }
}
                   ...
                   int beam_nb)
{
  ...
  int max_ch = 0;
  uint32_t nvar = 0;
  uint32_t nvar_per_rb[rel15_ul->rb_size];
  memset(nvar_per_rb, 0, sizeof(nvar_per_rb));
  ...
  for(uint8_t symbol = rel15_ul->start_symbol_index; symbol < end_symbol; symbol++) {
    ...
    if (dmrs_symbol_flag == 1) {
      ...
      for (int nl = 0; nl < rel15_ul->nrOfLayers; nl++) {
        uint32_t nvar_tmp = 0;
        uint32_t nvar_tmp_per_rb[rel15_ul->rb_size];
        memset(nvar_tmp_per_rb, 0, sizeof(nvar_tmp_per_rb));
        ...
        nvar += nvar_tmp;
        for (int rb = 0; rb < rel15_ul->rb_size; rb++)
          nvar_per_rb[rb] += nvar_tmp_per_rb[rb];
      }
  ...
  // Count actual DMRS symbols for correct normalization
  int num_dmrs_symbols = 0;
  for (int l = rel15_ul->start_symbol_index; l < end_symbol; l++)
    num_dmrs_symbols += ((rel15_ul->ul_dmrs_symb_pos >> l) & 0x01);
  if (num_dmrs_symbols > 0 && rel15_ul->nrOfLayers > 0) {
    int dmrs_divisor = num_dmrs_symbols * rel15_ul->nrOfLayers;
    nvar /= dmrs_divisor;
    for (int rb = 0; rb < rel15_ul->rb_size; rb++)
      nvar_per_rb[rb] /= dmrs_divisor;
  }

  // James-Stein shrinkage: blend per-RB and global nvar to reduce estimation noise
  // while preserving real frequency-selective variation.
  // In flat channels (AWGN/LOS), observed spread ≈ sampling noise → α→0 → uses global.
  // In frequency-selective channels, observed spread >> sampling noise → α→1 → keeps per-RB.
  if (nvar > 0 && rel15_ul->rb_size > 1) {
    uint64_t sum_sq_dev = 0;
    for (int rb = 0; rb < rel15_ul->rb_size; rb++) {
      int64_t dev = (int64_t)nvar_per_rb[rb] - (int64_t)nvar;
      sum_sq_dev += (uint64_t)(dev * dev);
    }
    uint64_t obs_var = sum_sq_dev / rel15_ul->rb_size;

    // Expected sampling variance of per-RB noise estimate: Var(σ̂²) ≈ 2σ⁴/n
    // where n = pilot samples per RB (6 for DMRS type 1) × antennas × DMRS symbols
    int n_samp = 6 * frame_parms->nb_antennas_rx * num_dmrs_symbols;
    uint64_t sampling_var = 2ULL * (uint64_t)nvar * (uint64_t)nvar / (n_samp > 0 ? n_samp : 1);

    // α in Q8: fraction of observed variance that is real channel variation
    int alpha_q8 = 0;
    if (obs_var > sampling_var)
      alpha_q8 = (int)(((obs_var - sampling_var) << 8) / obs_var);

    for (int rb = 0; rb < rel15_ul->rb_size; rb++)
      nvar_per_rb[rb] = (uint32_t)(((uint64_t)nvar_per_rb[rb] * alpha_q8
                                   + (uint64_t)nvar * (256 - alpha_q8)) >> 8);
  }
  ...
  // Interpolate channel estimates between DMRS symbols for data symbols
  if (gNB->chest_time == 0 && num_dmrs_symbols > 1)
    nr_chest_time_domain_interp(pusch_vars->ul_ch_estimates,
  ...
  int dmrs_symbol;
  if (gNB->chest_time == 0) {
    if (num_dmrs_symbols > 1)
      dmrs_symbol = meas_symbol; // interpolated estimates available at each symbol
    else
  ...
  // Store DMRS-based SINR components for CQI reporting
  uint64_t sum_avg = 0;
  for (int nl = 0; nl < rel15_ul->nrOfLayers; nl++)
    for (int aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++)
      sum_avg += avg[nl * frame_parms->nb_antennas_rx + aarx];
  pusch_vars->dmrs_signal_power = sum_avg;
  pusch_vars->dmrs_nvar = nvar;
  pusch_vars->dmrs_shift_ch_ext = shift_ch_ext;
