...
typedef struct {
  ...
  /// DMRS-based noise variance (correctly normalized)
  uint32_t dmrs_nvar;
  /// DMRS-based signal power (sum of per-antenna channel energy)
  uint64_t dmrs_signal_power;
  /// channel scaling shift for dB correction in SINR computation
  int16_t dmrs_shift_ch_ext;
  /// \brief llr values.
